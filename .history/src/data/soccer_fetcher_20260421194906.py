...
def _fbref_standard_stats(league_key: str, season: str = "2024-2025") -> pd.DataFrame:
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; research/educational use)"}
    try:
...
def _fbref_shooting_stats(league_key: str, season: str = "2024-2025") -> pd.DataFrame:
    ...
    url = (
        f"https://fbref.com/en/comps/{league_id}/{season}/shooting/"
        f"{season}-{league_key}-Stats"
    )
    headers = {"User-Agent": "Mozilla/5.0 (compatible; research/educational use)"}
    try:
...
def get_soccer_player_props_batch(
    ...
    # Load stats per league (respecting circuit-breaker)
