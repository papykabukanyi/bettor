...
try:
    ...
    MLB_API_OK = False
    print("[mlb_fetcher] mlbstatsapi not installed – pip install mlbstatsapi")
...
def get_schedule_today() -> list[dict]:
    ...
    try:
        ...
        schedule = mlb.get_schedule(start_date=today, end_date=today)
        games = []
        for date_obj in schedule.dates:
            for game in date_obj.games:
                home = game.teams.home.team.name
                away = game.teams.away.team.name
                # probable starters may not be set early in the day
                home_sp = getattr(
                    getattr(game.teams.home, "probablePitcher", None), "fullName", "TBD"
                )
                away_sp = getattr(
                    getattr(game.teams.away, "probablePitcher", None), "fullName", "TBD"
                )
                games.append({
                    "game_pk": game.gamePk,
                    "date": today,
