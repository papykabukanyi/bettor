...
def _get_odds(sports: list[str]) -> "pd.DataFrame":
    """Fetch live odds for a list of sport keys."""
    import pandas as pd
    from data.odds_fetcher import get_live_odds, odds_to_dataframe

    all_rows = []
    for sport in sports:
        raw = get_live_odds(sport)
        if raw:
...
def main():
    ...
    parser.add_argument("--leagues", nargs="+", default=["EPL", "ESP", "GER", "MLS"],
    ...
    odds_df = _get_odds(["mlb", "epl", "laliga", "bundesliga", "mls"])

    # Find value bets
    from analysis.value_finder import find_value_bets, build_parlay, summarise_suggestions

    value_bets = find_value_bets(
        mlb_preds,    odds_df[odds_df.get("sport_key", pd.Series()) == "mlb"] if not odds_df.empty else odds_df,
        sport="mlb"
    ) + find_value_bets(
        soccer_preds, odds_df[odds_df.get("sport_key", pd.Series()) != "mlb"] if not odds_df.empty else odds_df,
        sport="soccer"
    )

    parlays = build_parlay(value_bets)
    print("\n" + summarise_suggestions(value_bets, parlays))
