...
def _isolated_model_cache(tmp_path, monkeypatch):
    ...
    # _hf_recheck_state is ALSO module-level global state -- see
    # test_perps_model.py's identical fixture comment for why this needs
    # resetting the same way _model_cache does.
    alpaca_crypto_model._hf_recheck_state.update({"last_checked_at": 0.0, "checking": False})  # noqa: SLF001
    yield
    alpaca_crypto_model._model_cache.update({"model": None, "meta": None, "loaded_at": 0.0})  # noqa: SLF001
    alpaca_crypto_model._hf_recheck_state.update({"last_checked_at": 0.0, "checking": False})  # noqa: SLF001
...
def test_load_model_schedules_a_background_hf_recheck_when_something_is_already_cached(monkeypatch):
    alpaca_crypto_model._model_cache.update({"model": object(), "meta": {"trained_at": "t1"}, "loaded_at": time_module.time()})  # noqa: SLF001
    fired = server_common.threading.Event()
    monkeypatch.setattr(alpaca_crypto_model, "_schedule_hf_model_recheck", lambda: fired.set())

    alpaca_crypto_model.load_model()
...
def test_load_model_does_not_schedule_a_recheck_with_nothing_cached_yet(monkeypatch):
    ...
    monkeypatch.setattr(alpaca_crypto_model, "_schedule_hf_model_recheck", fail_if_called)
    alpaca_crypto_model.load_model()  # no model file on disk (isolated tmp_path) -- falls through to the None, None path


def test_schedule_hf_model_recheck_picks_up_a_newer_model_from_hf(monkeypatch):
    alpaca_crypto_model._model_cache.update({"model": "old-model-object", "meta": {"trained_at": "2026-01-01T00:00:00+00:00"}, "loaded_at": time_module.time()})  # noqa: SLF001
    monkeypatch.setattr(alpaca_crypto_model, "HF_API_KEY", "fake-key")
    monkeypatch.setattr(server_common, "pull_json_from_hf", lambda *a, **k: {"trained_at": "2026-02-01T00:00:00+00:00"})
    monkeypatch.setattr(alpaca_crypto_model, "_download_model_from_hf", lambda: True)

    alpaca_crypto_model._schedule_hf_model_recheck()  # noqa: SLF001
    for _ in range(50):
        if alpaca_crypto_model._model_cache["loaded_at"] == 0.0:  # noqa: SLF001
            break
        time_module.sleep(0.02)
    assert alpaca_crypto_model._model_cache["loaded_at"] == 0.0  # noqa: SLF001
...
def test_schedule_hf_model_recheck_leaves_the_cache_alone_when_hf_has_nothing_newer(monkeypatch):
    ...
    alpaca_crypto_model._model_cache.update({"model": "current-model-object", "meta": {"trained_at": "2026-01-01T00:00:00+00:00"}, "loaded_at": original_loaded_at})  # noqa: SLF001
    monkeypatch.setattr(alpaca_crypto_model, "HF_API_KEY", "fake-key")
    ...
    monkeypatch.setattr(alpaca_crypto_model, "_download_model_from_hf", fail_if_called)

    alpaca_crypto_model._schedule_hf_model_recheck()  # noqa: SLF001
    time_module.sleep(0.1)
    assert alpaca_crypto_model._model_cache["loaded_at"] == original_loaded_at  # noqa: SLF001
