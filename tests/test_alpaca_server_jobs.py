...
def test_threads_hourly_status_job_posts_open_positions(monkeypatch):
    ...
    from data import alpaca_strategy, threads_post

    opened_at = (dt.datetime.now(dt.timezone.utc) - dt.timedelta(minutes=15)).isoformat()
    monkeypatch.setattr(alpaca_strategy, "_load_state", lambda: {
        "positions": [{"symbol": "AAPL", "entry_price": 150.0, "count": 1.0, "opened_at": opened_at}],
    ...
    result = alpaca_server._run_alpaca_threads_hourly_status.__wrapped__()  # noqa: SLF001
...
def test_threads_hourly_status_job_never_raises_on_failure(monkeypatch):
    from data import alpaca_strategy
