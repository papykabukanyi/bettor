...
app = Flask("alpaca_crypto_server", template_folder=str(SRC_DIR / "templates"), static_folder=str(SRC_DIR / "static"))
...
#
# Real, confirmed production incident: a train job's data-loading phase
# (hundreds of individual per-shard HF network round trips, made
# meaningfully longer by the recent MAX_TRAIN_ROWS increase) ran for 7+
# minutes straight on the shared "default" single-worker pool, during
# which the equivalent options service's own fast_check -- the real-money
# exit/TP-SL/max-hold check for OPEN positions, meant to run every 20s --
# was completely blocked (confirmed live: "skipped: maximum number of
# running instances reached" repeating for the entire duration). perps
# already solved this exact problem for exactly this reason (see its own
...
# crypto trades 24/7, so this one has the least slack of any market here.
    ...
    with _startup_lock:
        ...
        if not scheduler.running and ENABLE_ALPACA_CRYPTO_SCHEDULER:
            ...
            scheduler.add_job(
                ...
                id="alpaca_crypto_fast_check", replace_existing=True, executor="fastcheck",
            )
            ...
            # at a stretch on the equivalent options service -- long enough
            # to blow past even the widened 300s misfire grace and get
            ...
            # api_alpaca_crypto_threads_trending_news and its 2 siblings
            ...
            # fix: with no offset, hourly_status/sentiment_snapshot (both
            # 60min) always land on the exact same tick, trending_news
            # (30min) coincides every other cycle, and data_collect (a
            # multiple of 15) piles on too.
            ...
            scheduler.add_job(
                ...
                id="alpaca_crypto_threads_hourly_status", replace_existing=True, executor="fastcheck",
            )
            scheduler.start()
            logger.info(
...
@app.route("/api/alpaca/crypto/threads/trending-news", methods=["GET", "POST"])
def api_alpaca_crypto_threads_trending_news():
    ...
    if not is_cron_authorized(request):
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    try:
        return jsonify(_run_alpaca_crypto_threads_trending_news())
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500


@app.route("/api/alpaca/crypto/threads/sentiment-snapshot", methods=["GET", "POST"])
def api_alpaca_crypto_threads_sentiment_snapshot():
    """Trigger route for an external scheduler -- see
    api_alpaca_crypto_threads_trending_news's own docstring."""
    if not is_cron_authorized(request):
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    try:
        return jsonify(_run_alpaca_crypto_threads_sentiment_snapshot())
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500


@app.route("/api/alpaca/crypto/threads/hourly-status", methods=["GET", "POST"])
def api_alpaca_crypto_threads_hourly_status():
    """Trigger route for an external scheduler -- see
    api_alpaca_crypto_threads_trending_news's own docstring."""
    if not is_cron_authorized(request):
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    try:
        return jsonify(_run_alpaca_crypto_threads_hourly_status())
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500
