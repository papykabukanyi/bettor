"""Direction classifier for Alpaca-traded crypto pairs -- trained on
...
on the older single-chronological-split contract perps_model.py moved
past (see perps_model.py's own git history: b4ddf4c "walk-forward CV,
calibration, and evidence-based ensembling", 8839a3a "post-trade win/
loss analysis + outcome-aware retraining" -- neither had a crypto-side
equivalent until now). This matters MORE here, not less: crypto's own
documented -13.7%/26.5%-win-rate backtest at current defaults (see
alpaca_crypto_backtest.py's own module-level comment) is exactly the
kind of thin/fragile edge that a single lucky-or-unlucky 80/20 split
could have overstated, and MODEL_CONFIDENCE_MIN=0.55 is compared
directly against probability_up -- a raw, uncalibrated sklearn score
before this change, same gap perps_model.py's own calibration step
closed. Deliberately does NOT change FEATURE_COLUMNS or touch
alpaca_crypto_strategy.py in this same pass -- an isolated, additive
training-pipeline upgrade first, so its own effect can be measured
before compounding it with other changes.
"""
...
ALPACA_CRYPTO_MODEL_RECENCY_HALFLIFE_DAYS = float(os.getenv("ALPACA_CRYPTO_MODEL_RECENCY_HALFLIFE_DAYS", "14") or "14")
...
ALPACA_CRYPTO_MODEL_CALIBRATION_MIN_HOLDOUT_ROWS = int(os.getenv("ALPACA_CRYPTO_MODEL_CALIBRATION_MIN_HOLDOUT_ROWS", "200") or "200")
...
# incidents on this exact service's old 512MB container (3 oomKilled
# restarts, roughly a day apart). Raised back up to 150 after migrating to
# a 2GB (standard plan) container -- same value already proven safe locally
# in this codebase's own backtest modules, not a new guess.
_CANDIDATES = {
...
class _AveragedEnsemble:
    ...
    a-priori rule. Identical to perps_model.py's own class of the same
    name -- replicated, not imported, matching this codebase's own
    per-market isolation convention (no cross-market imports)."""
...
ALPACA_CRYPTO_MODEL_TRADE_OUTCOME_WIN_WEIGHT = float(os.getenv("ALPACA_CRYPTO_MODEL_TRADE_OUTCOME_WIN_WEIGHT", "1.3") or "1.3")
ALPACA_CRYPTO_MODEL_TRADE_OUTCOME_LOSS_WEIGHT = float(os.getenv("ALPACA_CRYPTO_MODEL_TRADE_OUTCOME_LOSS_WEIGHT", "1.8") or "1.8")
...
def _trade_outcome_sample_weight(symbols: np.ndarray, ts: np.ndarray, trade_log: list[dict[str, Any]] | None) -> np.ndarray:
    ...
    more than wins. Identical design to perps_model.py's own function of
    the same name (see its docstring for the full rationale) -- keyed on
    symbol instead of ticker, otherwise unchanged. Every row not matching a
    real trade entry keeps weight 1.0, unaffected."""
    ...
    for i in range(len(symbols)):
        ...
        if won is not None:
            weights[i] *= ALPACA_CRYPTO_MODEL_TRADE_OUTCOME_WIN_WEIGHT if won else ALPACA_CRYPTO_MODEL_TRADE_OUTCOME_LOSS_WEIGHT
    return weights
...
def train_model(df: pd.DataFrame | None = None, trade_log: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    supports it -- persist locally + to HF_ALPACA_CRYPTO_MODEL_REPO.
    Returns a summary dict either way (never raises on ordinary "not
    enough data yet" conditions -- expected during the first days of data
    collection). See this module's own docstring for why this now mirrors
    perps_model.train_model()'s design exactly instead of the older single
    80/20-split contract.

    trade_log (alpaca_crypto_strategy state's own trade_log, passed in by
    the caller -- this module never imports alpaca_crypto_strategy
    ...
    # returns) mirrors perps_model.py's own real OOM-avoidance fix.
    del frame
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(splits):
        ...
        for name, factory in _CANDIDATES.items():
            try:
                ...
                logger.warning("[alpaca_crypto_model] candidate %s failed on fold %d: %s", name, fold_idx, exc)
    ...
    can_calibrate = (
        ...
        and len(last_test_idx) >= ALPACA_CRYPTO_MODEL_CALIBRATION_MIN_HOLDOUT_ROWS
    ...
    if can_calibrate:
        ...
        full_sample_weight = _recency_sample_weight(ts_all, half_life_days=ALPACA_CRYPTO_MODEL_RECENCY_HALFLIFE_DAYS)
    ...
    meta = {
        ...
        "recency_halflife_days": ALPACA_CRYPTO_MODEL_RECENCY_HALFLIFE_DAYS,
        "calibration_min_holdout_rows": ALPACA_CRYPTO_MODEL_CALIBRATION_MIN_HOLDOUT_ROWS,
