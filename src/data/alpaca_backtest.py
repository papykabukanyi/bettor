...
from data.alpaca_data import FEATURE_COLUMNS, load_training_dataset
...
def run_walkforward_backtest(
    *, fold_bounds: list[tuple[float, float, float]] | None = None,
    starting_balance: float = 10_000.0, max_shards: int = 90, **strategy_overrides: Any,
) -> dict[str, Any]:
    """Loads the same combined multi-symbol dataset alpaca_model.py's own
    train_model() uses (load_training_dataset()), but replays MULTIPLE
    expanding-window train/test folds (see walkforward.py) instead of a
    single 70/30 split -- see perps_backtest.run_walkforward_backtest's own
    docstring for why this matters more than a single split.

    Returns `{"ok", "folds": [...], "fold_count", "profitable_fold_ratio",
    "mean_return_pct", "std_return_pct", ...}`. Deliberately not called from
    _run_alpaca_intensive_training -- see this module's own real-OOM-history
    comment above _CANDIDATES; multiplying that job's already-tight fit cost
    by the fold count is not worth it for a job that already runs daily."""
