"""Meta-labeling for perps_model.py's own primary direction classifier: a
secondary, deliberately simple classifier predicting whether the PRIMARY
model's prediction is trustworthy IN THE CURRENT CONTEXT (time of day,
volatility/liquidity regime, and the primary model's own confidence) --
NOT a second opinion on direction, only on whether to trust the first
opinion.

Motivated by a real, confirmed limitation found in this session's own
audit of the live model: perps_model.py's walk-forward AUC sits around
~0.53-0.54 on ~98K training rows -- a genuinely thin edge -- and every
model-confirmed signal above MODEL_CONFIDENCE_MIN is currently trusted
EQUALLY regardless of context, even though a thin-edge classifier is
very unlikely to be equally reliable across every regime/hour/confidence
level it operates in. This can't manufacture edge that isn't there; it
can only help skip the primary model's worst contexts.
...
perps_model.py's latest training run picked (via the SAME `_CANDIDATES`
factory dict imported from perps_model.py), one fresh fit per
TimeSeriesSplit fold, so every "was this prediction right" label comes
from a model that never saw that row during ITS OWN training -- the same
leakage discipline perps_model.py's own walk-forward CV already holds
itself to. Deliberately does NOT call or modify perps_model.train_model()
itself -- this is a fully separate, additive training pass with zero
risk to the existing, live-critical primary training pipeline.

Off by default (PERPS_USE_META_MODEL, gated in perps_strategy.py) pending
real backtest validation via perps_backtest.py -- same evidence-gated
rollout discipline as every other new signal in this codebase (see e.g.
PERPS_ENABLE_SHORTS's own graduated-default history)."""
...
from data.perps_data import load_training_dataset
from data.perps_model import (
    PERPS_MODEL_RECENCY_HALFLIFE_DAYS,
    _CANDIDATES,
...
META_MODEL_PATH = DATA_DIR / "perps_meta_model.joblib"
META_MODEL_META_PATH = DATA_DIR / "perps_meta_model_meta.json"
...
# confident was the primary model" vector, all already-computed columns
# from perps_data.py's own feature frame except `primary_confidence`
# itself (derived below, not a raw column).
...
MIN_META_TRAIN_ROWS = int(os.getenv("PERPS_META_MODEL_MIN_TRAIN_ROWS", "500") or "500")
META_WALK_FORWARD_SPLITS = int(os.getenv("PERPS_META_MODEL_WALK_FORWARD_SPLITS", "4") or "4")
...
META_HOLDOUT_FRACTION = float(os.getenv("PERPS_META_MODEL_HOLDOUT_FRACTION", "0.2") or "0.2")

_meta_model_cache: dict[str, Any] = {"model": None, "meta": None, "loaded_at": 0.0}
META_MODEL_CACHE_TTL_SEC = int(os.getenv("PERPS_META_MODEL_CACHE_TTL_SEC", "1800") or "1800")
HF_META_MODEL_RECHECK_INTERVAL_SEC = int(os.getenv("PERPS_META_MODEL_HF_RECHECK_INTERVAL_SEC", "600") or "600")
...
def _build_oof_dataset(df: pd.DataFrame, *, model_type: str) -> pd.DataFrame | None:
    ...
    perps_model.train_model, but entirely standalone -- see this module's
    own docstring for why) refitting `model_type` per fold, and returns
    one row per out-of-fold test sample: this row's own context features
    plus whether the primary model's call on it was actually correct.
    None if there isn't enough data or `model_type` isn't a real
    candidate."""
    if model_type not in _CANDIDATES:
        logger.warning("[perps_meta_model] unknown primary model_type %r, skipping", model_type)
        return None
    labeled = _prepare_training_frame(df)
    if len(labeled) < MIN_META_TRAIN_ROWS:
        return None

    from data.perps_model import FEATURE_COLUMNS  # local import: avoids a module-load-order surprise if perps_model ever imports this module back
    ...
    for train_idx, test_idx in tscv.split(x_all):
        ...
        try:
            ...
            logger.warning("[perps_meta_model] fold fit failed: %s", exc)
            continue
...
def train_meta_model(df: pd.DataFrame | None = None, *, primary_model_type: str | None = None) -> dict[str, Any]:
    ...
    perps_strategy.py, not just a directional call), not maximally
    expressive. Evaluated on a chronological holdout the meta-model never
    trains on, for an honest accuracy/AUC in the returned summary. Never
    raises on ordinary "not enough data yet" conditions."""
    frame = df if df is not None else load_training_dataset()
    if frame.empty:
        return {"ok": False, "reason": "no_data"}

    if primary_model_type is None:
        from data.perps_model import load_model as _load_primary_model
        ...
        # perps_model.py's own docstring already treats as a fully valid
        # non-ensemble outcome (calibration is skipped the same way on
        # thin data).
    ...
    if len(holdout_part) >= 20 and len(set(holdout_part["was_correct"])) >= 2:
        ...
        # uncalibrated" posture as perps_model.py's own cold-start path --
        # refit on ALL out-of-fold rows rather than leaving a usable model
        # on the table over a too-small holdout slice.
...
def _push_meta_model_to_hf() -> None:
    ...
    try:
        ...
        from data.perps_model import retry_on_rate_limit
        ...
        retry_on_rate_limit(lambda: api.upload_file(
            path_or_fileobj=str(META_MODEL_PATH), path_in_repo="perps_meta_model.joblib",
            repo_id=HF_MODEL_REPO, repo_type="model", commit_message="update perps meta-model",
        ))
        retry_on_rate_limit(lambda: api.upload_file(
            path_or_fileobj=str(META_MODEL_META_PATH), path_in_repo="perps_meta_model_meta.json",
            repo_id=HF_MODEL_REPO, repo_type="model", commit_message="update perps meta-model metadata",
        ))
    except Exception as exc:
        logger.warning("[perps_meta_model] HF push failed: %s", exc)


_META_MODEL_DOWNLOAD_HF_TIMEOUT_SEC = int(os.getenv("PERPS_META_MODEL_DOWNLOAD_HF_TIMEOUT_SEC", "15") or "15")
...
def _download_meta_model_from_hf() -> bool:
    ...
    def _download() -> bool:
        ...
        model_path = hf_hub_download(repo_id=HF_MODEL_REPO, filename="perps_meta_model.joblib", repo_type="model", token=HF_API_KEY)
        meta_path = hf_hub_download(repo_id=HF_MODEL_REPO, filename="perps_meta_model_meta.json", repo_type="model", token=HF_API_KEY)
    ...
    try:
        ...
        logger.info("[perps_meta_model] no meta-model available on HF yet: %s", exc)
        return False
...
def _schedule_hf_meta_model_recheck() -> None:
    ...
    def _check() -> None:
        refresh_model_if_hf_has_a_newer_one(
            model_repo=HF_MODEL_REPO, token=HF_API_KEY, meta_filename="perps_meta_model_meta.json",
...
def load_meta_model() -> tuple[Any | None, dict[str, Any] | None]:
    """Same cache/download/staleness-recheck shape as perps_model.load_model
    -- see that function's own docstring for the full rationale (a
    meta-model retrained by a job that no longer runs in THIS process,
    e.g. scripts/training_job.py as an HF Job, still needs to be picked
    up on a reasonable cadence, not just on this process's next full
    restart)."""
    now = time.time()
    ...
    try:
        ...
        logger.warning("[perps_meta_model] failed to load cached meta-model: %s", exc)
        return None, None
...
def trust_score(row: dict[str, Any] | None, *, primary_probability_up: float) -> float | None:
    ...
    already computed by perps_model.predict_direction) is CORRECT --
    None if no meta-model is trained yet or `row` is missing the context
    features this needs. Never raises -- same best-effort contract as
    every other prediction path in this codebase."""
    if row is None:
        return None
    ...
    try:
        ...
        logger.debug("[perps_meta_model] trust_score failed: %s", exc)
        return None
