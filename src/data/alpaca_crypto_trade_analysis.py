...
# bot has as a resource" (the user's own request). Reuses the SAME
...
# triggered immediately when a sweep/walk-forward run reports a loss,
# instead of only reacting to the bot's own slower-accumulating real
# trade_log history.
#
# Deliberately scoped to MODEL_CONFIDENCE_MIN only, not also
# TAKE_PROFIT_PCT/STOP_LOSS_PCT despite the sweep varying those too: a real
# finding from this same review -- adaptive_exit_pcts() (see its own
# docstring) scales take-profit/stop-loss to each pair's OWN entry-time
# volatility for the overwhelming majority of real trades, and only falls
# back to the flat TAKE_PROFIT_PCT/STOP_LOSS_PCT constants when
# entry_volatility_30 is missing (a rare, near-cold-start-only case).
# alpaca_crypto_backtest.run_config_sweep's own "wider_take_profit" family
# of variants sets those SAME flat constants (see its own _SWEEP_GRID) --
# meaning a real difference in ITS OWN reported return_pct across those
# variants is very likely coming from whatever ELSE that variant also
# changed (model_confidence_min, entry_dip_pct, max_hold_minutes), not the
# TP/SL change itself, for live trading specifically (the backtest's own
# simulated trades take the SAME adaptive branch live trades do). Auto-
# applying a flat TP/SL "winner" here would misattribute credit and set an
# override that barely does anything live. Confidence, by contrast, is a
# plain threshold compared directly against the model's own output with no
...
def recommend_confidence_from_backtest(sweep_result: dict[str, Any] | None, *, current_threshold: float) -> dict[str, Any]:
    """Scans a alpaca_crypto_backtest.run_config_sweep() result for a
    variant that (a) sets a DIFFERENT model_confidence_min than what's
    live today, (b) has an adequately-sized, non-low_sample trade count,
    and (c) returned a meaningfully better return_pct than whatever
    variant reflects today's actual live config (matched by
    model_confidence_min, falling back to the sweep's own "current_defaults"
    label if present). Returns should_apply=False whenever the sweep
    result is missing/malformed, too thin to trust, or doesn't show a
    clear, adequately-sampled improvement -- same non-committal-by-default
    posture as recommend_confidence_threshold above."""
    configs = (sweep_result or {}).get("configs") or []
    if not configs:
