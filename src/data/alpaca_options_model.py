...
HF_MODEL_RECHECK_INTERVAL_SEC = int(os.getenv("ALPACA_OPTIONS_MODEL_HF_RECHECK_INTERVAL_SEC", "600") or "600")
...
class _AveragedEnsemble:
    ...
    def predict(self, x: np.ndarray) -> np.ndarray:
        # Real, confirmed live bug: this class only had predict_proba,
        # but the torch-vs-current-model re-score comparison calls
        # .predict() on whatever's currently deployed -- when an averaged
        # ensemble WAS the live model (it gets shipped whenever a real
        # walk-forward shows it beats every individual candidate), that
...
def _schedule_hf_model_recheck() -> None:
    ...
    def _check() -> None:
        refresh_model_if_hf_has_a_newer_one(
            model_repo=HF_ALPACA_OPTIONS_MODEL_REPO, token=HF_API_KEY, meta_filename="alpaca_options_model_meta.json",
