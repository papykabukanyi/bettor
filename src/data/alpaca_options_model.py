...
identical to alpaca_model.py/alpaca_crypto_model.py/perps_model.py: same
chronological-holdout-never-random-shuffle discipline, same candidate
comparison, same feature-importance surfacing. Trained on
alpaca_options_data's features/labels (the UNDERLYING's own direction) and
persisted to HF_ALPACA_OPTIONS_MODEL_REPO -- its own model, as explicitly
requested, even though the feature recipe matches the equities model
exactly (predicting whether a stock goes up or down is the same problem
whether the result is used to buy shares or buy an option on it).
"""
...
def train_model(df: pd.DataFrame | None = None) -> dict[str, Any]:
    ...
    persist locally + to HF_ALPACA_OPTIONS_MODEL_REPO. Never raises on
    ...
    for name, factory in _CANDIDATES.items():
        try:
            ...
            logger.warning("[alpaca_options_model] candidate %s failed: %s", name, exc)
