...
# pattern prevents): api_alpaca_options_status() called
# alpaca_client.get_account() -- a real, uncached Alpaca API round trip --
# on every single /api/alpaca/options/status poll (every 10s from the
# browser), unlike market_session just above, which already had this same
# caching. Balance/equity don't need millisecond freshness for a
# dashboard number.
    ...
    if (now - _ACCOUNT_SNAPSHOT_CACHE_TS) >= _ACCOUNT_SNAPSHOT_CACHE_TTL_SEC:
        try:
            ...
            if account.get("equity") is not None:
                milestones = alpaca_options_strategy.record_milestone(float(account["equity"]))
                save_json(ALPACA_OPTIONS_MILESTONES_FILE, milestones)
        except Exception as exc:
            logger.debug("[alpaca_options_server] background account cache refresh failed: %s", exc)
    return result
...
def api_alpaca_options_status():
    ...
    # cycle -- reused here via a symbol lookup, not a second fetch). Same
    # *100 contract multiplier the real exit-booking code uses.
    checks_by_symbol = {c["symbol"]: c for c in latest_position_check.get("checks") or [] if c.get("symbol")}
    positions = []
    for p in state.get("positions") or []:
        enriched = {**p, **alpaca_options_strategy.position_exit_levels(p)}
        ...
        if check and check.get("current_price") is not None:
            ...
            enriched["unrealized_pnl_usd"] = round((current_price - entry_price) * count * 100, 6)
    ...
    return jsonify({
        ...
        "milestones": load_json(ALPACA_OPTIONS_MILESTONES_FILE, {}),
        "underlyings": alpaca_options_data.get_options_universe(),
        "model": {
...
@app.route("/api/alpaca/options/threads/trending-news", methods=["GET", "POST"])
def api_alpaca_options_threads_trending_news():
    ...
    if not is_cron_authorized(request):
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    try:
        return jsonify(_run_alpaca_options_threads_trending_news())
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500


@app.route("/api/alpaca/options/threads/sentiment-snapshot", methods=["GET", "POST"])
def api_alpaca_options_threads_sentiment_snapshot():
    """Trigger route for an external scheduler -- see
    api_alpaca_options_threads_trending_news's own docstring."""
    if not is_cron_authorized(request):
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    try:
        return jsonify(_run_alpaca_options_threads_sentiment_snapshot())
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500


@app.route("/api/alpaca/options/threads/hourly-status", methods=["GET", "POST"])
def api_alpaca_options_threads_hourly_status():
    """Trigger route for an external scheduler -- see
    api_alpaca_options_threads_trending_news's own docstring."""
    if not is_cron_authorized(request):
        return jsonify({"ok": False, "error": "Unauthorized"}), 401
    try:
        return jsonify(_run_alpaca_options_threads_hourly_status())
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 500
